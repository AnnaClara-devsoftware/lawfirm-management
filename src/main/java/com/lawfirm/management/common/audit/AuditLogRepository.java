...
public interface AuditLogRepository extends JpaRepository<AuditLog, UUID> {
 @Query("select a from AuditLog a where (:action is null or a.action = :action) and (:entityName is null or lower(a.entityName)=lower(:entityName)) and (:userId is null or a.userId=:userId) and (:from is null or a.createdAt >= :from) and (:to is null or a.createdAt <= :to)")
